...
method~\cite{bendsoe2003}, which handles the single volume constraint
via bisection on the Lagrange multiplier $\Lambda$:
\begin{equation}
    ...
    \label{eq:oc}
\end{equation}
with damping exponent $\eta=0.5$ and move limit $0.2$.
The Lagrange multiplier is determined by bisection until the volume
constraint is satisfied to tolerance $10^{-4}$.
The Method of Moving Asymptotes (MMA)~\cite{svanberg1987} is also
implemented; a convergence comparison with OC is in Supplementary~S4.
OC is used for all main results.
...
The target fluid volume fraction $V^*$ decreases linearly from $0.7$ to
$0.5$ over the first 40 iterations.
This initially relaxed constraint allows the optimiser to establish a
well-connected channel network before removing excess material.
...
\]
every 30 iterations, driving the design from grey intermediate states
to near-binary ($<2\%$ grey volume at $\beta=64$).
...
\begin{equation}
    \alpha_{\max}(t) = 10^{\,3 + 6t},
    \qquad t = \min\!\left(1,\;\frac{\text{step}}{\text{max\_iter}}\right),
    \label{eq:alpha_cont}
\end{equation}
ensuring that fluid can flow through intermediate-density regions during
early iterations when the concentration field must develop.
