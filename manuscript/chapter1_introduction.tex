...
\texttt{micrograd} optimises within the Brinkman--SIMP porous-medium
model on engineering-resolution meshes ($\Delta y\approx25\,\mu$m).
The optimised permeability distribution is \emph{not} a manufacturable
design: hard-thresholding to binary permeability increases RMSE
by $+356\%$ (Section~\ref{sec:thresholding}), because the optimiser
exploits diffusive transport through intermediate-density regions
that have no physical analogue.
The paper's contribution is a documented, tested, and reproducible
open-source implementation; users requiring binary designs should
apply robust projection~\cite{wang2011} during optimisation,
which is identified as the principal future direction.
